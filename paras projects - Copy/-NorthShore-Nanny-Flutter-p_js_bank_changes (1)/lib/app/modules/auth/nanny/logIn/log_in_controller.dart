...
class LogInController extends GetxController {
      ...
      _apiHelper.postApi(ApiUrls.customerLogin, body).futureValue(
        ...
        if (userType == 2) {
          ...
          if (res.response == AppConstants.apiResponseSuccess) {
            ...
            } else if (res.data?.user?.isVenmoDetailsAdded == null) {
              RouteManagement.goToVenmoView();
          ...
          if (res.response == AppConstants.apiResponseSuccess) {
            ...
            if (res.data?.user?.isProfileCreated == false) {
              ...
              Storage.saveValue(StringConstants.isAppleLogin, false);
              Storage.removeValue(StringConstants.appleLogInFirstName);
              Storage.removeValue(StringConstants.appleLogInLastName);
  ...
  Future<void> socialLogInOrSignup({
    ...
    try {
      ...
      log('Apple First Name:$firstName');
      log('Apple Last Name:$lastName');
      ...
      _apiHelper.postApi(ApiUrls.socialLogIn, body).futureValue((value) async {
        var res = CustomerLoginResponseModel.fromJson(value);
        if (res.response == AppConstants.apiResponseSuccess) {
          Storage.saveValue(StringConstants.token, res.data!.token);
          Storage.saveValue(StringConstants.userId, res.data?.user!.id);
          ...
          Storage.saveValue(
              StringConstants.inviteAFriendCode, res.data?.user?.couponCode);
          if (res.data?.user?.isProfileCreated == false) {
            Storage.saveValue(StringConstants.isAppleLogin, true);
            Storage.saveValue(
                StringConstants.appleLogInFirstName, firstName ?? '');
            Storage.saveValue(
                StringConstants.appleLogInLastName, lastName ?? '');
            /** PROFILE is not created */
            RouteManagement.goToCreateCustomerProfile();
        ...
        log("Social Api have some Error please check $error ");
      }, retryFunction: () {});
    } catch (e, s) {
      toast(msg: e.toString(), isError: true);
  ...
  /* comment due to Not give the Meta Account
