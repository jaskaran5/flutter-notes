...
class GetNannyProfileController extends GetxController {
  ...
  bool isElementEqualToData(List<AvailabilityList> list, int day, int month) {
    for (var value in list) {
      final openingTime = value.openingTime;
      var dateTime =
      DateFormat("yyyy-MM-dd HH:mm:ss").parse(openingTime.toString(), true);
      var dateLocal = dateTime.toLocal();
      if (dateLocal.day == day && dateLocal.month == month) {
        return true;
      }
    }
    return false;
  ...
  updateSelectedDate({required DateTime date, required DateTime focusDate}) {
    ...
    selectChildrenTextController.clear();
    update();
  }
  ...
  usedToStoreNannyID() {
    if (Get.arguments != null ) {
      nannyId.value = Get.arguments['id'] ?? 0;
      isFromGuest.value = (Get.arguments as Map).containsKey("from") &&
  ...
  getNannyDetails({required DateTime time, int? nannyUserID}) async {
    ...
    try {
      ...
      _apiHelper
          .postApi(
              !isFromGuest.value
                  ? ApiUrls.nannyDetails
                  : "${ApiUrls.nannyDetailsForGuest}?UserId=${nannyUserID ?? nannyId.value}",
              body)
          .futureValue((value) {
        printInfo(info: "get nanny profile  response value $value");
...
//** GET CHILD LIST API */
  ...
  confirmBookingApi({
    ...
    try {
      ...
      var body = {
        ...
        "bookingDate": openingTime.toIso8601String(),
  ...
  returnTotalPrice({
    ...
    List<String> list = [];
    list.addAll(servicesList);
    list.removeWhere(
      (element) =>
          element.toString() == Services.funActivityOutHouse.serviceName.tr,
    );

    var value = list.length * 10 + double.parse(totalMinutesPrice);
    log('value:$value');
  ...
  returnServiceFeeAccordingToTotalPrice(
      {required var servicesList, required String totalMinutesPrice}) {
    List<String> list = [];
    list.addAll(servicesList);
    list.removeWhere(
      (element) =>
