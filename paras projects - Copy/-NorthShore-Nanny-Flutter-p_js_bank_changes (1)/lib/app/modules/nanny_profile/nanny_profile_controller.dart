      ...
      _apiHelper
        ...
        if (response.response == AppConstants.apiResponseSuccess) {
          ...
          if (!Get.isRegistered<SettingController>()) {
            ...
            settingController.nannyImage.value = response.data?.image ?? '';
            settingController.nannyName.value = response.data?.name ?? '';
            settingController.nannyEmail.value = response.data?.email ?? '';
            settingController.isVenmoDetailAdded.value =
                response.data?.isVenmoDetailsAdded ?? false;
      ...
      if (availabilityList.isNotEmpty) {
        _apiHelper
          ...
          if (response.response == AppConstants.apiResponseSuccess) {
            ...
            // selectedIndex = 1;
            getAvailabilityList(
                dateTime: selectedDate?.toUtc().toIso8601String().toString() ??
                    focusDate.toUtc().toIso8601String().toString());
            selectedDate = null;
            update();
          } else {
            toast(msg: response.message.toString(), isError: true);
    ...
    try {
      ...
      _apiHelper
        ...
        body,
          ...
          .futureValue((value) {
        AvailabilityByDayModel response = AvailabilityByDayModel.fromJson(value);
        if (response.response == AppConstants.apiResponseSuccess) {
          singleDay = response;
          update();
        } else {
