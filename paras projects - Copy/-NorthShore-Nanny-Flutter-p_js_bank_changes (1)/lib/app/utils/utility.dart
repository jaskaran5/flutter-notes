...
class Utility {
  ...
  static String convertDateToMMMMYYYEEE(DateTime date) {
    ...
    return DateFormat('MMMM yyyy, EEEE').format(dateLocal);
  }
  ...
  /// used to convert date time to this format '[12/03/2024']
  static String convertStringToDateFormatMMDDYY(String date) {
  ...
  static String getDay({required DateTime? dateTime}) {
    var convertedDateTime = DateFormat("yyyy-MM-dd HH:mm:ss")
        .tryParse(dateTime?.toString() ?? "", true);
    var dateLocal = convertedDateTime?.toLocal();
    return dateLocal?.day.toString().padLeft(2, '0') ?? '';
  }
